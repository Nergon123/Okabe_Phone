...
#pragma warning(disable : 4996) /* for Microsoft Visual C/C++ */
    ...
    defined __NetBSD__ || defined __DragonFly__ || defined __GNUC__
...
static TCHAR *skiptrailing(const TCHAR *str, const TCHAR *base) {
    assert(str != NULL);
    assert(base != NULL);
    while (str > base && '\0' < *(str - 1) && *(str - 1) <= ' ') { str--; }
    return (TCHAR *)str;
}
...
static TCHAR *ini_strncpy(TCHAR *dest, const TCHAR *source, size_t maxlen,
    ...
    if (option == QUOTE_ENQUOTE && maxlen < 3) {
        option = QUOTE_NONE; /* cannot store two quotes and a terminating zero in less than 3
                                characters */
    }

    switch (option) {
    case QUOTE_NONE:
        for (d = 0; d < maxlen - 1 && source[d] != '\0'; d++) { dest[d] = source[d]; }
        assert(d < maxlen);
        dest[d] = '\0';
        break;
    case QUOTE_ENQUOTE:
    ...
    case QUOTE_DEQUOTE:
        for (d = s = 0; source[s] != '\0' && d < maxlen - 1; s++, d++) {
            if ((source[s] == '"' || source[s] == '\\') && source[s + 1] == '"') { s++; }
            dest[d] = source[s];
        }
        dest[d] = '\0';
        break;
...
static TCHAR *cleanstring(TCHAR *string, enum quote_option *quotes) {
    ...
    for (ep = string; *ep != '\0' && ((*ep != ';' && *ep != '#') || isstring); ep++) {
        if (*ep == '"') {
            if (*(ep + 1) == '"') { ep++; /* skip "" (both quotes) */ }
            else { isstring = !isstring; /* single quote, toggle isstring */ }
        }
        else if (*ep == '\\' && *(ep + 1) == '"') { ep++; /* skip \" (both quotes */ }
    }
...
static int getkeystring(INI_FILETYPE *fp, const TCHAR *Section, const TCHAR *Key, int idxSection,
                        int idxKey, TCHAR *Buffer, int BufferSize, INI_FILEPOS *mark) {
    TCHAR            *sp, *ep;
    int               len, idx;
    ...
    if (len > 0 || idxSection >= 0) {
        ...
        do {
            do {
                if (!ini_read(LocalBuffer, INI_BUFFERSIZE, fp)) { return 0; }
        ...
        while ((((int)(ep - sp) != len || Section == NULL || _tcsnicmp(sp, Section, len) != 0) &&
                ++idx != idxSection));
        if (idxSection >= 0) {
            if (idx == idxSection) {
                assert(ep != NULL);
 ...
 */
int ini_gets(const TCHAR *Section, const TCHAR *Key, const TCHAR *DefValue, TCHAR *Buffer,
             int BufferSize, const TCHAR *Filename) {
    INI_FILETYPE fp;
    int          ok = 0;

    if (Buffer == NULL || BufferSize <= 0 || Key == NULL) { return 0; }
...
int ini_getbool(const TCHAR *Section, const TCHAR *Key, int DefValue, const TCHAR *Filename) {
    ...
    if (LocalBuffer[0] == 'Y' || LocalBuffer[0] == '1' || LocalBuffer[0] == 'T') { ret = 1; }
    else if (LocalBuffer[0] == 'N' || LocalBuffer[0] == '0' || LocalBuffer[0] == 'F') { ret = 0; }
    else { ret = DefValue; }
...
int ini_getsection(int idx, TCHAR *Buffer, int BufferSize, const TCHAR *Filename) {
    ...
    if (Buffer == NULL || BufferSize <= 0 || idx < 0) { return 0; }
    if (ini_openread(Filename, &fp)) {
...
int ini_browse(INI_CALLBACK Callback, void *UserData, const TCHAR *Filename) {
    TCHAR             LocalBuffer[INI_BUFFERSIZE];
    int               lenSec, lenKey, SectionIndex;
    enum quote_option quotes;
    INI_FILETYPE      fp;
    ...
    for (;;) {
        TCHAR *sp, *ep;
        if (!ini_read(LocalBuffer + lenSec, INI_BUFFERSIZE - lenSec, &fp)) { break; }
        ...
        if (ep == NULL) { ep = _tcschr(sp, ':'); }
        if (ep == NULL) { continue; /* invalid line, ignore */ }
        *ep++ = '\0'; /* split the key from the value */
        ...
        /* call the callback */
        if (!Callback(LocalBuffer, LocalBuffer + lenSec, LocalBuffer + lenSec + lenKey,
                      SectionIndex, UserData)) {
            break;
        }
    }
...
static enum quote_option check_enquote(const TCHAR *Value) {
     ...
     */
    assert(Value != NULL);
    for (p = Value; *p != '\0' && *p != '"' && *p != ';' && *p != '#'; p++) /* nothing */;
...
static void writesection(TCHAR *LocalBuffer, const TCHAR *Section, INI_FILETYPE *fp) {
    if (Section != NULL && _tcslen(Section) > 0) {
        ...
        *p++ = ']';
        ...
        if (fp != NULL) { (void)ini_write(LocalBuffer, fp); }
    }
}
...
static int cache_accum(const TCHAR *string, int *size, int max) {
    ...
    if (*size + len >= max) { return 0; }
    *size += len;
    return 1;
}

static int cache_flush(TCHAR *buffer, int *size, INI_FILETYPE *rfp, INI_FILETYPE *wfp,
                       INI_FILEPOS *mark) {
    ...
    if (buffer[0] != '\0') {
        ...
        if (pos == INI_BUFFERSIZE) { pos--; }
        buffer[pos] =
            '\0'; /* force zero-termination (may be left unterminated in the above while loop) */
 ...
 */
            ...
            if (_tcslen(LocalBuffer) == (size_t)(tail - head)) {
                 ...
                 */
                (void)ini_close(&rfp);
                if (!ini_openrewrite(Filename, &wfp)) { return 0; }
        ...
        /* Conversely, for a request to delete a setting; if that setting isn't
           present, just return */
        match =
            getkeystring(&rfp, Section, Key, -1, -1, LocalBuffer, sizearray(LocalBuffer), NULL);
        if (!match) {
            (void)ini_close(&rfp);
            return 1;
        }
    ...
    for (;;) {
        ...
        if (Key == NULL) {
            (void)ini_tell(
                &rfp, &mark); /* we are deleting the entire section, so update the read position */
        }
        else {
            if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
                cache_flush(LocalBuffer, &cachelen, &rfp, &wfp, &mark);
    ...
    flag = (*sp == '[');
    cache_flush(LocalBuffer, &cachelen, &rfp, &wfp, &mark);
    if (Key != NULL && Value != NULL) { writekey(LocalBuffer, Key, Value, &wfp); }
