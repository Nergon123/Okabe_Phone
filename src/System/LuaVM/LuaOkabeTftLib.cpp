...
extern "C" {
	...
	static int tft_simple_print(lua_State *L, int isnewline)
	{
		...
		for (i=1; i<=n; i++)
		{
			// if (i>1) printf("\t");
			if (lua_isstring(L,i))
				tft.printf("%s",lua_tostring(L,i));
			else if (lua_isnil(L,i))
				tft.printf("%s","nil");
			else if (lua_isboolean(L,i))
				tft.printf("%s",lua_toboolean(L,i) ? "true" : "false");
			else
				tft.printf("%s:%p",luaL_typename(L,i),lua_topointer(L,i));
		}
	...
	{
		return tft_simple_print(L, 0);
	}

	static int tft_println(lua_State *L)
	{
		return tft_simple_print(L, 1);
	}
