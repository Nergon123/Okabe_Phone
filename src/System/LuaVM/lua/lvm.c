...
static void callTMres(lua_State *L, StkId res, const TValue *f, const TValue *p1,
                      const TValue *p2) {
...
void luaV_gettable(lua_State *L, const TValue *t, TValue *key, StkId val) {
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++) {
        ...
        else if (ttisnil(tm = luaT_gettmbyobj(L, t, TM_INDEX))) { luaG_typeerror(L, t, "index"); }
        if (ttisfunction(tm)) {
...
int luaV_lessthan(lua_State *L, const TValue *l, const TValue *r) {
    int res;
    if (ttype(l) != ttype(r)) { return luaG_ordererror(L, l, r); }
    else if (ttisnumber(l)) { return luai_numlt(nvalue(l), nvalue(r)); }
    else if (ttisstring(l)) { return l_strcmp(rawtsvalue(l), rawtsvalue(r)) < 0; }
    else if ((res = call_orderTM(L, l, r, TM_LT)) != -1) { return res; }
...
static int lessequal(lua_State *L, const TValue *l, const TValue *r) {
    int res;
    if (ttype(l) != ttype(r)) { return luaG_ordererror(L, l, r); }
    else if (ttisnumber(l)) { return luai_numle(nvalue(l), nvalue(r)); }
    else if (ttisstring(l)) { return l_strcmp(rawtsvalue(l), rawtsvalue(r)) <= 0; }
...
static void Arith(lua_State *L, StkId ra, const TValue *rb, const TValue *rc, TMS op) {
    ...
    else if (!call_binTM(L, rb, rc, ra, op)) { luaG_aritherror(L, rb, rc); }
}
...
reentry: /* entry point */
    ...
    for (;;) {
        ...
        case OP_UNM: {
            ...
            else { Protect(Arith(L, ra, rb, rb, TM_UNM)); }
            continue;
        }
        case OP_NOT: {
        ...
        case OP_VARARG: {
            ...
            for (j = 0; j < b; j++) {
                if (j < n) { setobjs2s(L, ra + j, ci->base - n + j); }
                else { setnilvalue(ra + j); }
            }
            continue;
        }
        }
