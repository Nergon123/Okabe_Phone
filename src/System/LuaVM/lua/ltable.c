...
static void setnodevector(lua_State *L, Table *t, int size) {
    ...
    if (size == 0) {                       /* no elements to hash part? */
        ...
        int i;
        lsize = ceillog2(size);
        if (lsize > MAXBITS) { luaG_runerror(L, "table overflow"); }
        size    = twoto(lsize);
...
static void resize(lua_State *L, Table *t, int nasize, int nhsize) {
    ...
    if (nasize < oldasize) { /* array part must shrink? */
        ...
        for (i = nasize; i < oldasize; i++) {
            if (!ttisnil(&t->array[i])) { setobjt2t(L, luaH_setnum(L, t, i + 1), &t->array[i]); }
        }
        /* shrink array */
    ...
    for (i = twoto(oldhsize) - 1; i >= 0; i--) {
        Node *old = nold + i;
        if (!ttisnil(gval(old))) { setobjt2t(L, luaH_set(L, t, key2tval(old)), gval(old)); }
    }
    if (nold != dummynode) { luaM_freearray(L, nold, twoto(oldhsize), Node); /* free old array */ }
}
...
const TValue *luaH_getstr(Table *t, TString *key) {
    ...
    do { /* check whether `key' is somewhere in the chain */
        if (ttisstring(gkey(n)) && rawtsvalue(gkey(n)) == key) { return gval(n); /* that's it */ }
        else { n = gnext(n); }
    }
    while (n);
    return luaO_nilobject;
...
const TValue *luaH_get(Table *t, const TValue *key) {
    ...
    default: {
        Node *n = mainposition(t, key);
        do { /* check whether `key' is somewhere in the chain */
            if (luaO_rawequalObj(key2tval(n), key)) { return gval(n); /* that's it */ }
            else { n = gnext(n); }
        }
        while (n);
        return luaO_nilobject;
...
TValue *luaH_set(lua_State *L, Table *t, const TValue *key) {
    ...
    t->flags        = 0;
    if (p != luaO_nilobject) { return cast(TValue *, p); }
    else {
