...
static void removeentry(Node *n) {
    ...
    if (iscollectable(gkey(n))) { setttype(gkey(n), LUA_TDEADKEY); /* dead key; remove it */ }
}
...
static int traversetable(global_State *g, Table *h) {
    ...
    while (i--) {
        ...
        if (ttisnil(gval(n))) { removeentry(n); /* remove empty entries */ }
        else {
...
static void traverseproto(global_State *g, Proto *f) {
    ...
    for (i = 0; i < f->sizeupvalues; i++) { /* mark upvalue names */
        if (f->upvalues[i]) { stringmark(f->upvalues[i]); }
    }
    for (i = 0; i < f->sizep; i++) { /* mark nested protos */
        if (f->p[i]) { markobject(g, f->p[i]); }
    }
    for (i = 0; i < f->sizelocvars; i++) { /* mark local-variable names */
        if (f->locvars[i].varname) { stringmark(f->locvars[i].varname); }
    }
}
...
static void GCTM(lua_State *L) {
    ...
    else { g->tmudata->gch.next = udata->uv.next; }
...
static l_mem singlestep(lua_State *L) {
    ...
    case GCSpropagate: {
        if (g->gray) { return propagatemark(g); }
    ...
    case GCSfinalize: {
        if (g->tmudata) {
            GCTM(L);
            if (g->estimate > GCFINALIZECOST) { g->estimate -= GCFINALIZECOST; }
...
void luaC_step(lua_State *L) {
    ...
    if (lim == 0) { lim = (MAX_LUMEM - 1) / 2; /* no limit */ }
    g->gcdept += g->totalbytes - g->GCthreshold;
    do {
        lim -= singlestep(L);
        if (g->gcstate == GCSpause) { break; }
    }
    while (lim > 0);
...
void luaC_barrierf(lua_State *L, GCObject *o, GCObject *v) {
    ...
    /* must keep invariant? */
    if (g->gcstate == GCSpropagate) { reallymarkobject(g, v); /* restore invariant */ }
    else {               /* don't mind */
