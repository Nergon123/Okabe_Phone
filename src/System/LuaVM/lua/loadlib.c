...
static void pusherror(lua_State *L) {
    ...
    char buffer[128];
    if (FormatMessageA(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, NULL, error, 0,
...
static void *ll_load(lua_State *L, const char *path) {
    ...
    if (ret == NSObjectFileImageSuccess) {
        NSModule mod = NSLinkModule(
            img, path, NSLINKMODULE_OPTION_PRIVATE | NSLINKMODULE_OPTION_RETURN_ON_ERROR);
...
static int readable(const char *filename) {
  ...
  return 1;
#endif
    return lua_okabe_readable(filename);
}
...
static int loader_Lua(lua_State *L) {
    ...
    if (luaL_loadfile(L, filename) != 0) { loaderror(L, filename); }
...
static int loader_C(lua_State *L) {
    ...
    if (ll_loadfunc(L, filename, funcname) != 0) { loaderror(L, filename); }
    return 1; /* library loaded successfully */
}
