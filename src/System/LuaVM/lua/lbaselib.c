...
static int luaB_newproxy(lua_State *L) {
    ...
    if (lua_toboolean(L, 1) == 0) { return 1; /* no metatable */ }
...
static int costatus(lua_State *L, lua_State *co) {
    ...
    case 0: {
        ...
        else if (lua_gettop(co) == 0) { return CO_DEAD; }
        else { return CO_SUS; /* initial state */ }
    }
...
static int auxresume(lua_State *L, lua_State *co, int narg) {
    ...
    if (!lua_checkstack(co, narg)) { luaL_error(L, "too many arguments to resume"); }
    if (status != CO_SUS) {
    ...
    if (status == 0 || status == LUA_YIELD) {
        ...
        if (!lua_checkstack(L, nres + 1)) { luaL_error(L, "too many results to resume"); }
...
static int luaB_corunning(lua_State *L) {
    if (lua_pushthread(L)) { lua_pushnil(L); /* main thread is not a coroutine */ }
    return 1;
}
