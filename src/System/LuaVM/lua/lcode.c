...
void luaK_nil(FuncState *fs, int from, int n) {
    ...
    if (fs->pc > fs->lasttarget) { /* no jumps to current position? */
        if (fs->pc == 0) {         /* function start? */
            if (from >= fs->nactvar) { return; /* positions are already clean */ }
        }
        else {
            ...
            if (GET_OPCODE(*previous) == OP_LOADNIL) {
                ...
                if (pfrom <= from && from <= pto + 1) { /* can connect both? */
                    if (from + n - 1 > pto) { SETARG_B(*previous, from + n - 1); }
                    return;
                }
            }
        }
...
static Instruction *getjumpcontrol(FuncState *fs, int pc) {
    ...
    if (pc >= 1 && testTMode(GET_OPCODE(*(pi - 1)))) { return pi - 1; }
    else { return pi; }
}
...
static int patchtestreg(FuncState *fs, int node, int reg) {
    ...
    if (reg != NO_REG && reg != GETARG_B(*i)) { SETARG_A(*i, reg); }
...
void luaK_patchlist(FuncState *fs, int list, int target) {
    if (target == fs->pc) { luaK_patchtohere(fs, list); }
    else {
...
static void freeexp(FuncState *fs, expdesc *e) {
    if (e->k == VNONRELOC) { freereg(fs, e->u.s.info); }
}
...
static void exp2reg(FuncState *fs, expdesc *e, int reg) {
    ...
    if (e->k == VJMP) { luaK_concat(fs, &e->t, e->u.s.info); /* put this jump in `t' list */ }
    if (hasjumps(e)) {
...
void luaK_setlist(FuncState *fs, int base, int nelems, int tostore) {
    ...
    if (c <= MAXARG_C) { luaK_codeABC(fs, OP_SETLIST, base, b, c); }
    else {
