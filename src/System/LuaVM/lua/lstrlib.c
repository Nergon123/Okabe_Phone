...
static int str_dump(lua_State *L) {
    ...
    lua_settop(L, 1);
    luaL_buffinit(L, &b);
    if (lua_dump(L, writer, &b) != 0) { luaL_error(L, "unable to dump given function"); }
    luaL_pushresult(&b);
    return 1;
}
...
static void push_onecapture(MatchState *ms, int i, const char *s, const char *e) {
    if (i >= ms->level) {
        ...
        else { luaL_error(ms->L, "invalid capture index"); }
    }
    else {
        ptrdiff_t l = ms->capture[i].len;
        if (l == CAP_UNFINISHED) { luaL_error(ms->L, "unfinished capture"); }
        if (l == CAP_POSITION) { lua_pushinteger(ms->L, ms->capture[i].init - ms->src_init + 1); }
        else { lua_pushlstring(ms->L, ms->capture[i].init, l); }
    }
}
...
static void add_s(MatchState *ms, luaL_Buffer *b, const char *s, const char *e) {
    ...
    for (i = 0; i < l; i++) {
        if (news[i] != L_ESC) { luaL_addchar(b, news[i]); }
            ...
            else if (news[i] == '0') { luaL_addlstring(b, s, e - s); }
            else {
...
static int str_gsub(lua_State *L) {
    ...
    luaL_Buffer b;
    luaL_argcheck(
        L, tr == LUA_TNUMBER || tr == LUA_TSTRING || tr == LUA_TFUNCTION || tr == LUA_TTABLE, 3,
...
static int str_format(lua_State *L) {
    ...
    while (strfrmt < strfrmt_end) {
        if (*strfrmt != L_ESC) { luaL_addchar(&b, *strfrmt++); }
            ...
            default: { /* also treat cases `pnLlh' */
                return luaL_error(L, "invalid option " LUA_QL("%%%c") " to " LUA_QL("format"),
                                  *(strfrmt - 1));
            }
            }
