...
static void check_match(LexState *ls, int what, int who, int where) {
    if (!testnext(ls, what)) {
        if (where == ls->linenumber) { error_expected(ls, what); }
            ...
            luaX_syntaxerror(
                ls, luaO_pushfstring(ls->L, LUA_QS " expected (to close " LUA_QS " at line %d)",
...
static int searchvar(FuncState *fs, TString *n) {
    ...
    for (i = fs->nactvar - 1; i >= 0; i--) {
        if (n == getlocvar(fs, i).varname) { return i; }
    }
    return -1; /* not found */
}
...
static int singlevaraux(FuncState *fs, TString *n, expdesc *var, int base) {
    if (fs == NULL) {                   /* no more levels? */
        ...
        if (v >= 0) {
            ...
            if (singlevaraux(fs->prev, n, var, 0) == VGLOBAL) { return VGLOBAL; }
...
static void fornum(LexState *ls, TString *varname, int line) {
    ...
    checknext(ls, ',');
    exp1(ls); /* limit */
    if (testnext(ls, ',')) { exp1(ls); /* optional step */ }
    else { /* default step = 1 */
...
static void ifstat(LexState *ls, int line) {
    ...
    else { luaK_concat(fs, &escapelist, flist); }
...
static void localstat(LexState *ls) {
    ...
    int     nexps;
    expdesc e;
    do { new_localvar(ls, str_checkname(ls), nvars++); }
    while (testnext(ls, ','));
    if (testnext(ls, '=')) { nexps = explist1(ls, &e); }
    else {
        e.k   = VVOID;
        nexps = 0;
    }
