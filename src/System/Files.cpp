...
String fileBrowser(String path, String format, bool graphical) {
    NFile dir(path);
    ...
    mOption opts[2];
    int count = 0;
    if (sdAvailable && SD.open("/")) opts[count++] = {"MicroSD card", Image(R_FILE_MANAGER_ICONS), 2};
    if (spiffsAvailable && SPIFFS.open("/")) opts[count++] = {"Integrated memory", Image(R_FILE_MANAGER_ICONS), 3};

    if (count == 0) return "";

    int ch = graphical ? listMenu(opts, count, false, 2, "FILE MANAGER")
                       : listMenuNonGraphical(opts, count, "FILE MANAGER");
    ...
    do {
        ...
        while (entry) {
            ...
            if (entry.isDirectory()) {
                ...
                String fileLow = name; fileLow.toLowerCase();
        ...
        choice = graphical ? listMenu(options, i, false, 2, "FILE MANAGER")
                           : listMenuNonGraphical(options, i, "FILE MANAGER");
