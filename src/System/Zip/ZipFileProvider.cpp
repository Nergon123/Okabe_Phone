...
int ZipFileProvider::setFile(NString *path)
{
    return setFile(path->c_str());
}
...
int ZipFileProvider::setFile(const std::string &path)
{
    ...
    res = UNZIP::getFileInfo(&m_fileInfo, NULL, NULL, NULL, NULL, NULL, NULL);
...
{
    if(m_isFileOpen) {
        return readCurrentFile((uint8_t *)buf, len);
    } else {
        return 0;
    }
}
...
{
    if(m_isFileOpen) {
        return getCurrentFilePos() < m_fileInfo.uncompressed_size;
    } else {
        return false;
    }
}
