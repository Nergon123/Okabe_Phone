...
ZipPackage::ZipPackage() { m_pkgZipProvider = new ZipFileProvider(); }
...
int ZipPackage::setZip(IFile *file) {
    ...
    if (res < 0) { return ZIPPKG_BADZIPERR; }

    res = m_pkgZipProvider->setFile("control.ini");
    if (res < 0) { return ZIPPKG_BADCONTROLERR; }
    ...
    if (m_manifestParser->ParseError()) { return ZIPPKG_BADCONTROLERR; }

    m_info.name = m_manifestParser->GetStringUnquoted("General", "name", "");
    if (m_info.name.empty()) { return ZIPPKG_BADCONTROLERR; }

    m_info.id = m_manifestParser->GetStringUnquoted("General", "id", "");
    if (m_info.id.empty()) { return ZIPPKG_BADCONTROLERR; }

    m_info.version = m_manifestParser->GetStringUnquoted("General", "version", "");
    if (m_info.version.empty()) { return ZIPPKG_BADCONTROLERR; }
