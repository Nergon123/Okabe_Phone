...
static __inline__
    __attribute__((always_inline)) void minimp3_cpuid(int CPUInfo[], const int InfoType) {
#if defined(__PIC__)
...
static int          have_simd() { /* TODO: detect neon for !MINIMP3_ONLY_SIMD */ return 1; }
#else /* SIMD checks... */
#define HAVE_SSE  0
#define HAVE_SIMD 0
#ifdef MINIMP3_ONLY_SIMD
...
static int hdr_valid(const uint8_t *h) {
    return h[0] == 0xff && ((h[1] & 0xF0) == 0xf0 || (h[1] & 0xFE) == 0xe2) &&
           (HDR_GET_LAYER(h) != 0) && (HDR_GET_BITRATE(h) != 15) && (HDR_GET_SAMPLE_RATE(h) != 3);
}
...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t  *hdr,
    ...
    if (HDR_IS_LAYER_1(hdr)) {
        ...
        else if (kbps >= 96 && sample_rate_idx != 1) { nbands = 30; }
    }
...
static void L12_read_scale_info(const uint8_t *hdr, bs_t *bs, L12_scale_info *sci) {
    ...
    for (i = 0; i < sci->total_bands; i++) {
        ...
        sci->bitalloc[2 * i] = ba;
        if (i < sci->stereo_bands) { ba = ba_code_tab[get_bits(bs, ba_bits)]; }
    ...
    for (i = sci->stereo_bands; i < sci->total_bands; i++) { sci->bitalloc[2 * i + 1] = 0; }
}
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr) {
    ...
    else { main_data_begin = get_bits(bs, 8 + gr_count) >> gr_count; }
...
static void L3_read_scalefactors(uint8_t *scf, uint8_t *ist_pos, const uint8_t *scf_size,
                                 const uint8_t *scf_count, bs_t *bitbuf, int scfsi) {
    int i, k;
    for (i = 0; i < 4 && scf_count[i]; i++, scfsi *= 2) {
        int cnt = scf_count[i];
        if (scfsi & 8) { memcpy(scf, ist_pos, cnt); }
        else {
            int bits = scf_size[i];
            if (!bits) {
                memset(scf, 0, cnt);
...
static void L3_decode_scalefactors(const uint8_t *hdr, uint8_t *ist_pos, bs_t *bs,
                                   const L3_gr_info_t *gr, float *scf, int ch) {
...
static void L3_huffman(float *dst, bs_t *bs, const L3_gr_info_t *gr_info, const float *scf,
                       int layer3gr_limit) {
    ...
    static const uint8_t tab32[]          = {130, 162, 193, 209, 44,  28, 76,  140, 9,   9,
    ...
    static const int16_t tabindex[2 * 16] = {0,    32,   64,   98,   0,    132,  180,  218,
...
#define BSPOS ((bs_next_ptr - bs->buf) * 8 - 24 + bs_sh)
    ...
    uint32_t       bs_cache =
        (((bs_next_ptr[0] * 256u + bs_next_ptr[1]) * 256u + bs_next_ptr[2]) * 256u +
         bs_next_ptr[3])
        << (bs->pos & 7);
            ...
            do {
                ...
                do {
                    ...
                    for (j = 0; j < 2; j++, dst++, leaf >>= 4) {
                        ...
                        else { *dst = g_pow43[16 + lsb - 16 * (bs_cache >> 31)] * one; }
                        FLUSH_BITS(lsb ? 1 : 0);
                    }
                    CHECK_BITS;
                }
...
static void L3_stereo_top_band(const float *right, const uint8_t *sfb, int nbands,
                               int max_band[3]) {
    int i, k;
...
static void L3_stereo_process(float *left, const uint8_t *ist_pos, const uint8_t *sfb,
    ...
    for (i = 0; sfb[i]; i++) {
        ...
        else if (HDR_TEST_MS_STEREO(hdr)) { L3_midside_stereo(left, sfb[i]); }
        left += sfb[i];
    }
}

static void L3_intensity_stereo(float *left, uint8_t *ist_pos, const L3_gr_info_t *gr,
                                const uint8_t *hdr) {
...
static void L3_antialias(float *grbuf, int nbands) {
    static const float g_aa[2][8] = {{0.85749293f, 0.88174200f, 0.94962865f, 0.98331459f,
...
static void L3_imdct_gr(float *grbuf, float *overlap, unsigned block_type, unsigned n_long_bands) {
    ...
    if (block_type == SHORT_BLOCK_TYPE) { L3_imdct_short(grbuf, overlap, 32 - n_long_bands); }
    else {
...
static void L3_save_reservoir(mp3dec_t *h, mp3dec_scratch_t *s) {
    ...
    if (remains > 0) { memmove(h->reserv_buf, s->maindata + pos, remains); }
    h->reserv = remains;
}
...
static void mp3d_synth_granule(float *qmf_state, float *grbuf, int nbands, int nch,
                               mp3d_sample_t *pcm, float *lins) {
    int i;
...
static int mp3d_find_frame(const uint8_t *mp3, int mp3_bytes, int *free_format_bytes,
                           int *ptr_frame_bytes) {
    int i, k;
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm,
    ...
    if (info->layer == 3) {
        ...
        for (i = 0, igr = 0; igr < 3; igr++) {
            if (12 == (i += L12_dequantize_granule(scratch.grbuf[0] + i, bs_frame, sci,
                                                   info->layer | 1))) {
                i = 0;
...
void mp3dec_f32_to_s16(const float *in, int16_t *out, int num_samples) {
    ...
    for (; i < aligned_count; i += 8) {
        ...
        b = VADD(b, VSET(0.5f));
        pcma =
            vqmovn_s32(vqaddq_s32(vcvtq_s32_f32(a), vreinterpretq_s32_u32(vcltq_f32(a, VSET(0)))));
        pcmb =
            vqmovn_s32(vqaddq_s32(vcvtq_s32_f32(b), vreinterpretq_s32_u32(vcltq_f32(b, VSET(0)))));
    ...
    for (; i < num_samples; i++) {
        ...
        if (sample >= 32766.5) { out[i] = (int16_t)32767; }
        else if (sample <= -32767.5) { out[i] = (int16_t)-32768; }
        else {
            int16_t s = (int16_t)(sample + .5f);
            s -= (s < 0); /* away from zero, to be compliant */
            out[i] = s;
