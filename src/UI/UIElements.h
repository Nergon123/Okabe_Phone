...
bool   button(String title, int xpos, int ypos, int w, int h, bool selected = false, int *direction = nullptr, bool usable = true, void *callback(void *params) = nullptr);
void   sNumberChange(int x, int y, int w, int h, int &val, int min, int max, bool selected = false, int *direction = nullptr, const char *format = "%02d", bool usable = true, void *callback(void *params) = nullptr);
